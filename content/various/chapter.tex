...
\section{Optimization tricks}
	...
	\subsection{Pragmas}
		\begin{itemize}
			\item \lstinline{#pragma GCC optimize ("Ofast")} (try O3) will make GCC auto-vectorize loops and optimizes floating points better.
			\item \lstinline{#pragma GCC optimize ("unroll-loops")} loop unrolling.
			\item \lstinline{#pragma GCC target ("avx2")} can double performance of vectorized code, but causes crashes on old machines.
			\item \lstinline{#pragma GCC optimize ("trapv")} kills the program on integer overflows (but is really slow).
			\item \lstinline{#pragma GCC target("avx2,bmi,bmi2,lzcnt,popcnt")} Extra.
		\end{itemize}
